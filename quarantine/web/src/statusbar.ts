...
export function hideChordIndicator(): void {
  ...
  if (el) el.remove();
  const toast = document.querySelector("#chordIndicatorToast");
...
function getOrCreateChordEl(): HTMLElement {
  ...
  let toast = document.querySelector<HTMLElement>("#chordIndicatorToast");
  if (!toast) {
    toast = document.createElement("div");
    toast.id = "chordIndicatorToast";
    toast.className = "chord-indicator-toast";
    document.body.appendChild(toast);
  }
  const el = document.createElement("span");
  el.id = "statusBarChord";
  el.className = "status-bar-chord toast-style";
