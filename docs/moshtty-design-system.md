...
| `--color-text-subtle`       | Hints, timestamps, disabled. Light mode `#6d6d75` (≥ 4.5:1 on `--color-app-bg` / `--color-workspace-bg`); dark mode `#80808a` (≥ 4.5:1 on `--color-app-bg` while staying above `--color-text-muted` luminance).        |
...
| `--color-accent`            | Primary action fill (monochrome: matches `--color-text-main` in each theme; not a brand hue).                                                                                                                          |
| `--color-accent-soft`       | Soft neutral surface for chips and selected controls (matches `--color-sidebar-bg-active`).                                                                                                                            |
| `--color-accent-on`         | Foreground on filled primary surfaces (`--color-workspace-bg` light, `--color-app-bg` dark).                                                                                                                           |
| `--color-success`           | Connected, healthy.                                                                                                                                                                                                    |
| `--color-warning`           | Degraded, intermediate.                                                                                                                                                                                                |
| `--color-danger`            | Error, lost pane.                                                                                                                                                                                                      |
| `--color-focus`             | Focus ring color (monochrome: matches `--color-text-main`; status uses success/warning/danger only).                                                                                                                   |
