...
| M7 Real remote acceptance     | Ready for review   | Reload reattach, companion-restart lost-pane recovery, and remote `moshttyctl pane split` app-layout control are verified against the Mac companion      |
| M8 UI Refresh                 | Ready for review   | Redesigned app layout, minimal dark/light aesthetic, custom window controls, unified top bar, collapsible projects sidebar, visual tests passing         |
