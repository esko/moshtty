...
| M8b UI Polish                 | In progress      | Audit on 2026-05-27 found ref-parity gaps and user-flagged interaction issues; follow-up scoped to `docs/agents/2026-05-27-8c-moshtty-ui-followup.md`. See `docs/visual-qa/8b/M8b-design-gap-assessment.md` (Opus verification section).                                                                                                 |
| M9b Command Palette           | Ready for review | `Ctrl+K` overlay: search and invoke any registered app action by name; keyboard-navigable; builds on existing `keymap.ts` registry                                                                                                                                                                                                       |
| M8c UI Followup               | Planned          | Reference parity follow-up to M8b: top-bar restructure (panel-left icon + overflow + left-aligned tabs), pane chrome → hover pills + full-area terminal, settings dialog discipline, dashboard/sidebar tone, light terminal token fix, project edit modal, visual QA refresh. Brief: `docs/agents/2026-05-27-8c-moshtty-ui-followup.md`. |
...
| Moshtty UI Polish (8b)                    | Cursor agent | In progress        | `docs/agents/2026-05-27-8b-moshtty-ui-polish.md` (follow-up `docs/agents/2026-05-27-8c-moshtty-ui-followup.md` covers remaining ref-parity gaps) |     |
| Command Palette (9b)                      | Cursor agent | Ready for review   | `docs/agents/2026-05-27-9-command-palette.md`                                                                                                    |     |
| Moshtty UI Followup (8c)                  | (unassigned) | Planned            | `docs/agents/2026-05-27-8c-moshtty-ui-followup.md`                                                                                               |     |
...
- M9b Command Palette (2026-05-27, Ready for review): Implementation present in working tree on `feat/moshtty-scaffold` (uncommitted). Delivered: `open-command-palette` action (`Ctrl+K`) in `keymap.ts`; `appHandlers.ts` hook extracting handler map from `App.tsx`; `CommandPalette.tsx` + `CommandPalette.css` (search filter, ↑/↓/Enter/Escape, mouseOnly rows disabled with tooltip, focus return, `role="dialog"` / `listbox` / `option`); wired in `App.tsx` with `paletteOpen` state. Palette CSS uses existing `--z-dialog`, `--font-size-caption`, and token contract (no new token edits). **Verification gap:** close-out agent ran `git diff` only — no Vitest/Playwright/agent-browser runs; no dedicated palette unit tests; `pnpm verify:full` pending. **Follow-ups:** coordinator runs full verify, agent-browser QA for 5 palette states (`docs/visual-qa/9b/`), axe-core on open palette, optional palette component tests, then commits atomic slice.
