...
| M8b UI Polish                 | Planned | BETA badge removal, neutral buttons, edge-to-edge panes, terminal theme picker (6 presets), OpenCode-style hybrid pill tabs                              |
| M9b Command Palette           | Planned | `Ctrl+K` overlay: search and invoke any registered app action by name; keyboard-navigable; builds on existing `keymap.ts` registry                       |
...
| Moshtty UI Polish (8b)                    | (unassigned) | Planned            | `docs/agents/2026-05-27-8b-moshtty-ui-polish.md`                                                        |     |
| Command Palette (9b)                      | (unassigned) | Planned            | `docs/agents/2026-05-27-9-command-palette.md`                                                           |     |
...
- M8b UI Polish (2026-05-27, Planned): Task brief written at `docs/agents/2026-05-27-8b-moshtty-ui-polish.md`. 14 visual reference screenshots collected in `docs/visual-qa/8b/references/` (4× OpenCode canonical, OpenCode tab bar pill reference, 3× Antigravity live). Renderer-only slice: BETA badge removal, neutral primary buttons, edge-to-edge terminal panes with hairline split handles, six-preset terminal theme picker via localStorage, and OpenCode-style hybrid tab strip (inactive tabs bare, active tab single filled pill). Agent-browser QA with mandatory screenshot table and axe-core required before Ready for review.
- M9b Command Palette (2026-05-27, Planned): Task brief written at `docs/agents/2026-05-27-9-command-palette.md`. Renderer-only feature (`Ctrl+K` overlay). Builds on existing `keymap.ts` `APP_ACTIONS` registry — no IPC, no main-process, no Go changes. Scope: `CommandPalette` component + CSS, `appHandlers.ts` hook extraction, `open-command-palette` action added to keymap, wired into `App.tsx`. 7 source files, at soft cap. Token gap check for `--z-overlay` and `--font-size-caption` required before implementing CSS.
